...
setwd("~/machine_learning/KEGG_GSVA/KEGG")
...
data_c1 <- as.matrix(data)[,na.omit(match(c1,colnames(data)))]
data_c2 <- as.matrix(data)[,na.omit(match(c2,colnames(data)))]
...
saveRDS(es.matrix,file="c2.KEGG.rds")
...
saveRDS(t_results,file="t_results_c2.KEGG.rds")
...
library(pheatmap)
rownames(t_results) <- gsub("KEGG_", "", rownames(t_results))
